...
def format_chip(runway, colour=True):
    ...
    allocation). Format: `🕐 Xh working @ avg` — remaining active Claude
    hours at the average weekly burn rate. Colour tiers: neutral ≥8h,
    yellow <8h, red <3h. The chip is the wk-cap planning signal:
    "I have N working hours left before the cap, regardless of when reset
    lands." Always shown when defined — not gated on wall-clock projection.
    """
    if runway is None:
        return None
    ...
    if runway >= 10:
        raw = f"\U0001F550 {int(round(runway))}h working @ avg"
    else:
        half_steps = round(runway * 2)
        rounded = half_steps / 2
        if rounded <= 0:
            raw = "\U0001F550 0h working @ avg"
        elif half_steps % 2 == 0:
            raw = f"\U0001F550 {int(rounded)}h working @ avg"
        else:
            raw = f"\U0001F550 {rounded}h working @ avg"
