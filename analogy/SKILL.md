...
**Option-block contract — scene then moves (REQUIRED whenever the answer has ≥2 options):**

When the answer presents choices, the analogy ships as **two beats** at the end of the block, never one:

1. **Set the scene (shared context anchor).** One short line establishing the single situation all the options live inside — the dish, the moment, the corner of the kitchen everyone is standing in. This is the frame the reader needs *before* the options mean anything.
2. **One move per option, inside that scene.** Each option rendered as a distinct action within the *same* established scene — one line each, parallel structure, so the *jump* between #1 and #2, #2 and #3 is felt in kitchen terms.

Both beats are mandatory. Three states:
- ❌ **Scene-only** — a single framing paragraph that covers all options together but never differentiates them. The reader learns the metaphor, but the technical table still does all the picking. (This was the original 2026-05-26 failure.)
- ❌ **Disconnected moves** — per-option one-liners with no shared scene, so the options read as unrelated metaphors instead of choices within one situation. (The gap this contract closes, per Mark 2026-06-13.)
- ✅ **Scene + moves** — establish the context once, then each option as a move within it.

Worked shape (Mark's own example, 2026-06-13): *"We've written the recipe card; the question is what a cook does when a taste goes off — **(1)** call the chef and wait, **(2)** quietly fix it, or **(3)** scrape the pan and start over — with a hard cap on how many times they're allowed to just-fix-it before they have to shout."* One scene (taste went off), three differentiated moves inside it.
